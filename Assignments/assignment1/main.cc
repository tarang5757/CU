...
//my  own tests
// Test 1: Reservations should not overlap
  Date d1(2022,9,30);
  Date d2(2022,9,28);
  Reservation reservation1("Cristiano ronaldo", d1, 3);
  Reservation reservation2("Lionel messi", d2, 2);
  if (!resOverlap(reservation1,reservation2,false)) score = 0;

  //test 2: Reservations overlap at the end
  Date d3(2022,10,3);
  Date d4(2022,10,1);
  Reservation r3("France ", d3, 3);
  Reservation r4("Argentina", d4, 5);
  if (!resOverlap(r3,r4,true)) score = 0;

  //test 3: Reservations overlap at the start
  Date d5(2022,10,3);
  Date d6(2022,10,5);
  Reservation r5("Kylian mbappe", d5, 3);
  Reservation r6("Erling haaland", d6, 2);
  if (!resOverlap(r5,r6,true)) score = 0;

  //test 4: Reservations composed within each other
  Date d7(2022,10,1);
  Date d8(2022,9,30);
  Reservation r7("PSG", d7, 4);
  Reservation r8("France", d8, 3);
  if (!resOverlap(r7,r8,true)) score = 0;
